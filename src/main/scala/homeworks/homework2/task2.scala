...
object task2 extends App {
  ...
  def traverse[A](list: List[Option[A]]): Option[List[A]] = {
    if (list.exists(i => i.isEmpty))
      None
    else
      Some(list.map(i => i.get))
  }
  ...
  def splitEither[A, B](list: List[Either[A, B]]): (List[A], List[B]) = {
    (list collect {case Left(value) => value},
      list collect {case Right(value) => value})
  }
  ...
  def validate[A, B](list: List[Either[A, B]]): Either[List[A], List[B]] = {
    val (left, right) = splitEither(list)
    if (right.nonEmpty && left.isEmpty)
      Right(right)
    else
      Left(left)
  }
